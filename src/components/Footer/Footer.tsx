...
const Footer = () => {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if (!email || !email.includes("@")) {
      setStatusMessage({ text: "Please enter a valid email address.", success: false });
      return;
    }
    ...
    try {
      const derivedName = email.split("@")[0].replace(/[._-]/g, " ") || "Subscriber";
      const res = await fetch("https://wadmin.syscorp.in/api/newsletter/subscribe", {
        method: "POST",
        headers: {
          "accept": "*/*",
      ...
      if (res.ok && data.success) {
        setStatusMessage({ text: data.message || "Thanks for subscribing to our newsletter!", success: true });
        setEmail("");
      } else {
        setStatusMessage({ text: data.message || "Failed to subscribe. Please try again.", success: false });
      }
    } catch (err) {
      console.error("Newsletter subscription error:", err);
      setStatusMessage({ text: "Network error. Please try again later.", success: false });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[3.2fr_2.2fr_2.6fr_4fr] gap-10 mb-14">
          ...
          <div className="flex flex-col gap-4">
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col gap-2 mt-2">
              <div className="flex bg-[#050814] border border-white/12 rounded-full px-5 py-1 gap-2.5 items-center h-[52px] transition-colors focus-within:border-blue-500">
                <input
                  type="email"
                  value={email}
                  ...
                  disabled={loading}
                  className="bg-transparent border-none outline-none text-white text-[14px] flex-1 min-w-0 placeholder:text-white/40 disabled:opacity-50"
                  required
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="w-[42px] h-[42px] bg-[#1A5CDD] border-none rounded-full flex items-center justify-center text-white cursor-pointer transition-all duration-200 hover:scale-[1.05] hover:bg-[#154ebc] disabled:opacity-60 disabled:hover:scale-100 flex-shrink-0"
              ...
              {statusMessage && (
                <p className={`text-[13px] font-medium m-0 px-2 transition-all ${statusMessage.success ? "text-emerald-400" : "text-rose-400"}`}>
                  {statusMessage.text}
                </p>
              )}
            </form>

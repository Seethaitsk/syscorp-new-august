...
export default function HeroSlider() {
  ...
  // Typewriter states with enhanced tech terms
  const [typedText, setTypedText] = useState("");
  const [wordIndex, setWordIndex] = useState(0);
  const [isDeleting, setIsDeleting] = useState(false);
  const [typingSpeed, setTypingSpeed] = useState(140);
  const words = ["Custom Websites", "AI Automation", "Enterprise CRM", "SEO Growth", "Cloud Systems"];
  const colors = ["#38BDF8", "#60A5FA", "#34D399", "#A78BFA", "#0361efff"];
  ...
  return (
    <div ref={containerRef}>
      ...
      {/* ─── BANNER CONTAINER WITH AMBIENT VIDEO BACKDROP ─── */}
      <section id="home-banner" className="sky-banner-sec bg-[#02071f] relative overflow-hidden text-white">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <canvas
            ...
            style={{ display: "block" }}
          />
          {/* Dark vignette readability overlays */}
          <div className="absolute inset-0 bg-gradient-to-b from-[#02071f]/80 via-transparent to-[#02071f]/90" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_40%,transparent_30%,#02071f_90%)]" />
        </div>
        ...
        <div style={{ position: "relative", zIndex: 5, display: "flex", flexDirection: "column", alignItems: "center", padding: "0 24px" }}>
          ...
          {/* Heading with Animated Typewriter Text */}
          <h1 className="sky-banner-heading relative z-10">
            We Build Digital Solutions
            <br />
            <span className="block mt-1 text-slate-300 font-light" style={{ fontSize: "0.62em", letterSpacing: "0.01em" }}>Specialised in</span>
            <span
              className="inline-block font-black transition-all duration-500"
              style={{
                color: colors[wordIndex],
                textShadow: `0 0 30px ${colors[wordIndex]}55, 0 0 60px ${colors[wordIndex]}30`,
                minWidth: "200px",
              }}
            >{typedText}</span>
            <span
              className="font-thin"
              style={{ color: colors[wordIndex], animation: "skyBlink 1s step-end infinite" }}
            >|</span>
          </h1>
